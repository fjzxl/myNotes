...
|:---:|:---|:---|:---|:---|:---:|
| 1 | 三角不等式 | $\lvert a+b\rvert \le \lvert a\rvert+\lvert b\rvert$ | $ab\ge 0$ | 任意范数下，绕行不划算 | [二](#二证明基础不等式) |
| 2 | 均值不等式（AM-GM） | $\sqrt[n]{a_1\cdots a_n} \le \dfrac{a_1+\cdots+a_n}{n}$（$a_i\ge 0$） | 全部相等 | 总量固定，均衡时乘积（体积）最大 | [二](#二证明基础不等式) |
| 3 | 排序不等式 | 逆序和 $\le$ 乱序和 $\le$ 同序和 | 每个逆序对都"捆绑"（一侧相等） | 同序匹配总产出最大，错配损失 = 双差乘积 | [二](#二证明基础不等式) |
| 4 | 切比雪夫不等式 | $\bar a\,\bar b \le \overline{ab}$（同序时，均值的乘积 $\le$ 乘积的均值） | 任一组全相等 | 同序 $\Rightarrow$ 协方差非负 | [二](#二证明基础不等式) |
| 5 | 柯西-施瓦茨 | $\bigl(\sum a_ib_i\bigr)^2 \le \bigl(\sum a_i^2\bigr)\bigl(\sum b_i^2\bigr)$ | $a \propto b$ | 功 $\le$ 力 $\times$ 位移；投影 $\le$ 模长；$\lvert\rho\rvert\le 1$ | [三](#三证明核心工具不等式) |
| 6 | 伯努利 | $(1+x)^r \ge 1+rx$（$x>-1$） | $r=1$ 或 $x=0$ | 切线在凸曲线下方；单利 $\le$ 复利 | [三](#三证明核心工具不等式) |
| 7 | 琴生 | $f\bigl(\sum\lambda_ix_i\bigr) \le \sum\lambda_if(x_i)$（$f$ 凸） | 全部相等（严格凸时） | 质心不会跳出凸图形；$x^2$ 特例 = 平行轴定理 | [三](#三证明核心工具不等式) |
| 8 | Nesbitt | $\sum\limits_{cyc}\dfrac{a}{b+c} \ge \dfrac32$ | $a=b=c$ | $\tfrac32$ 是最均衡状态的最低消费 | [三](#三证明核心工具不等式) |
| 9 | Young | $ab \le \dfrac{a^p}{p}+\dfrac{b^q}{q}$（$\tfrac1p+\tfrac1q=1$） | $a^p=b^q$ | $L^p$–$L^q$ 对偶的单点版：预算由两种范数分摊 | [四](#四证明高阶不等式) |
| 10 | Hölder | $\sum a_ib_i \le \bigl(\sum a_i^p\bigr)^{1/p}\bigl(\sum b_i^q\bigr)^{1/q}$ | $a_i^p \propto b_i^q$ | 耦合效应 $\le$ 容量之积（$L^p$–$L^q$ 对偶） | [四](#四证明高阶不等式) |
| 11 | 权方和 | $\sum\dfrac{a_i^{p+1}}{b_i^p} \ge \dfrac{(\sum a_i)^{p+1}}{(\sum b_i)^p}$ | $\dfrac{a_i}{b_i}$ 全相等 | 并联电路最小功耗：按容量分流；分开除比合并除贵 | [四](#四证明高阶不等式) |
| 12 | Minkowski | $\lVert a+b\rVert_p \le \lVert a\rVert_p+\lVert b\rVert_p$（$p\ge1$） | 同号且成比例 | 单位球凸 $\Leftrightarrow$ 三角不等式成立 | [四](#四证明高阶不等式) |
| 13 | 指数与对数 | $e^x\ge 1+x$；$\dfrac{x-1}{x}\le\ln x\le x-1$ | $x=0$ / $x=1$ | 切线界；KL 散度非负：信息不无中生有 | [六](#六应用) |
| 14 | 三角函数 | $\sin x<x<\tan x$；Jordan：$\tfrac{2}{\pi}x\le\sin x\le x$ | 仅区间端点 | 单位圆面积序；大角度单摆变慢 | [六](#六应用) |
| 15 | 积分形式（四件套） | C-S / 琴生 / Hölder / Minkowski 的"和式 $\to$ 积分"版本 | 与离散版一致 | 函数 = 无穷维向量，同一套几何 | [六](#六应用) |

**目录**：

- [二、证明：基础不等式](#二证明基础不等式)：三角不等式 · 均值不等式 · 排序不等式 · 切比雪夫
- [三、证明：核心工具不等式](#三证明核心工具不等式)：柯西-施瓦茨 · 伯努利 · 琴生 · Nesbitt
- [四、证明：高阶不等式](#四证明高阶不等式)：Young → Hölder → 权方和 / Minkowski（递推链）
- [五、多视角理解](#五多视角理解向量矩阵几何物理)：向量·矩阵·几何·物理透镜下逐个重看，一句话本质
- [六、应用](#六应用)：指数与对数 · 三角函数 · 积分形式（四件套）· 题型速查
...
分点、圆心与半弦上端构成直角三角形（直径所对圆周角为直角），斜边是半径、直角边是半弦，"斜边 $\ge$ 直角边"正是 $\sqrt{ab} \le \dfrac{a+b}{2}$——**AM-GM 就是直角三角形的斜边不短于直角边**。取等 ⟺ 分点与圆心重合 ⟺ $a=b$。
...
切比雪夫不等式其实就是 **"同方向变化的两个量,它们的乘积被加权放大"**。下面从四个角度把它说透。
...
把数据各自中心化，$\tilde a=(a_i-\bar a)$、$\tilde b=(b_i-\bar b)$ 看成两个向量，则
$$
...
- **物理视角（统计力学·变分原理）**：Gibbs–Bogoliubov 变分原理：任取一个试探态，"期望能量减温度乘试探熵"永远不低于真实自由能，$F \le \langle H\rangle - TS$——无论试探态选得多差。它的证明核心正是 $\ln$ 的凹性（琴生）/ KL 散度非负（见应用篇指数对数一节）。定温系统落在自由能最低处，而"任何近似都给不出低于真值的自由能"这层保险，是琴生不等式打的。
...
- **几何视角（$L^p$ 单位球族）**：
  ...
  把"长度"换成 $p$-范数后，单位球从菱形（$p=1$）逐渐胖成圆（$p=2$）再到方形（$p\to\infty$）。Hölder 的几何内容：**$L^p$ 单位球在对偶方向上被 $L^q$ 单位球卡住**（共轭的 $q$ 恰是 $p$ 的"极"——$p=1$ 的对偶图形正是 $p=\infty$ 的方形，$p=2$ 自对偶）。C-S 里"投影不超过模长"的图景，推广为"支撑不超过对偶球的卡位"；等号 ⟺ 向量落在对偶方向上。
...
- **信息论视角**：由 $\ln t\le t-1$ 一行推出 **Gibbs 不等式（KL 散度非负）**——对 $t=\frac{q_i}{p_i}$ 使用并翻转：
  ...
  $$
  即**交叉熵 ≥ 熵**——"信息不会无中生有"，这是整个信息论与最大似然理论的基石，而它的源头就是应用篇那条最朴素的对数不等式。
...
- **向量视角**：积分就是连续内积——把函数看成无穷维向量，$\int fg$ 是内积、$\int f^2$ 是模方。应用篇积分形式一节的四个积分不等式与离散形式**逐字对应**：C-S（函数的投影不等式）、Jensen（连续质量分布的质心仍在凸集内）、Hölder（$L^p$–$L^q$ 对偶）、Minkowski（$L^p$ 范数的三角不等式）。记住"和式 $\to$ 积分"，两套只需记一套。
...
证明与离散版逐字平行：对被积函数逐点用 Young 不等式再标准化，把求和换成积分即可（参见[四、证明：Hölder 不等式](#四证明高阶不等式)）。
...
同样把离散证明（用 Hölder 拆 $|f+g|^p$ 为两项）逐字翻译成积分即可。
