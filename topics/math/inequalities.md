...
先用 $n=2$ 把两边完整展开，看清每一项从哪来。左边的 $L$ 是**两个和的乘积**，展开规则是"各取一项相乘、所有组合都要"：
...
共 $2\times 2=4$ 项。注意 $(1,2)$ 与 $(2,1)$ 是**两个不同的项**：$a_1^2b_2^2$ 一般不等于 $a_2^2b_1^2$。右边的 $R$ 是**和的平方**，展开同样取所有组合 $(a_ib_i)(a_jb_j)$，但乘法可交换——$(1,2)$ 与 $(2,1)$ 两项数值相同，合并成系数 $2$ 的交叉项：

$$
R = (a_1b_1 + a_2b_2)^2 = \underbrace{a_1^2b_1^2}_{(1,1)} + \underbrace{2a_1b_1a_2b_2}_{(1,2)+(2,1)\text{ 合并}} + \underbrace{a_2^2b_2^2}_{(2,2)}
$$

两式相减：对角项 $(1,1)$、$(2,2)$ 一减一零、恰好抵消，只剩

$$
a_1^2b_2^2 + a_2^2b_1^2 - 2a_1b_1a_2b_2 = (a_1b_2 - a_2b_1)^2 .
$$

一般的 $n$ 是同一个故事，只是按"下标对"分组。$L$ 展开的 $n^2$ 项 $a_i^2b_j^2$ 与 $R$ 展开的 $n^2$ 项 $a_ib_ia_jb_j$ 相减后：
...
- **物理视角（统计力学·变分原理）**：Gibbs–Bogoliubov 变分原理：任取一个试探态，"期望能量减温度乘试探熵"永远不低于真实自由能，$F \le \langle H\rangle - TS$——无论试探态选得多差。它的证明核心正是 $\ln$ 的凹性（琴生）/ KL 散度非负（见指数对数一节）。定温系统落在自由能最低处，而"任何近似都给不出低于真值的自由能"这层保险，是琴生不等式打的。
