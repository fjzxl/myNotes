# 工具
- [工具](#工具)
  - [shell](#shell)
  - [快捷键](#快捷键)
    - [Markdown All in One](#markdown-all-in-one)
    - [vscode 快捷键](#vscode-快捷键)
  - [链接](#链接)
  - [知识管理工具](#知识管理工具)
    - [Foam（推荐）](#foam推荐)
  - [相关工具笔记](#相关工具笔记)

## shell

## 快捷键

### Markdown All in One

1. 生成目录
  使用 Command Shift P > content
  Markdown All in One: Create Table of Contents \
  Markdown All in One: Update Table of Contents

2. 快捷键

  |   快捷键   |   命令     |
  |   ----    |   ----    |
  | Ctrl/Cmd + B |  切换加粗  |
  | Alt+S (Windows)  | 切换删除线|
  | Ctrl/Cmd + I  | 切换斜体  |
  | Ctrl/Cmd + M  | 切换数学环境  |
  | Alt + C | 勾选/取消勾选任务列表项 |
  | Ctrl/Cmd + Shift + V  | 切换预览  |
  | Ctrl/Cmd + K V  | 切换侧边预览  |

### vscode 快捷键

- Alt + 点击： 多游标
- Ctrl + Shift + P： 命令面板
- Ctrl + N/O/S/F/H： 新建、打开、保存、查找、替换

## 链接

* [VSCode配置大全](https://www.lianxh.cn/details/1004.html)

## 知识管理工具

### Foam（推荐）

- [[topics/tools/foam-knowledge-base-guide]] — 完整使用指南
- 入口页：[[topics/tools/Foam]]
- 一句话：VS Code 插件，Roam 式双向链接知识库
- 适用：本仓库所有 Markdown 笔记可作为 Foam 知识库使用

## 相关工具笔记

- [[topics/tools/VSCode]] — VS Code 配置与快捷键完整指南
- [[topics/tools/Markdown]] — Markdown 写作环境配置
- [[topics/tools/Foam]] — 知识库管理工具索引
- [[topics/tools/marp-guide]] — Marp 幻灯片（Markdown → PDF/PPT）